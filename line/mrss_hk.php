...
require "../configA.php";
	...
	$lang = ( (!isset($_GET['lang']) ) || ($_GET['lang'] == '') ) ? 'hk' : $_GET['lang'];
	$feed = array( 'hk' => array('K6LAMBU4P2TWKUIEARGB','mp4') );
	$sdate30=strtotime(date('Y-m-d h:i:sa', strtotime('-120 hours')));
	$edate30=strtotime(date('Y-m-d h:i:sa'));
	$dateChar = '&filters[]=c_ts_publish_l:['.$sdate30.'%20TO%20'.$edate30.']';
	// BEGIN Syndicate Now

	$getUrls2Now= APPLICATION_FEED_URL.$feed[$lang][0].'?start=0&count=100'.$dateChar.'&sort=c_ts_publish_l+desc';
	$ww=curl_info($getUrls2Now, '', '');
	$dAry22=json_decode($ww, true);
    $dAryn2=$dAry22['docs'];
    //print_r($dAry22);
	generate_xml($dAryn2, $lang, $feed[$lang][1]);
...
function generate_xml($dAryn2, $lang, $typeContent){
		...
		$titleCont = $xml_doc->createTextNode($uuid.$lang.$dAryn2[0]['obj_id']);
		...
		foreach ($dAryn2 as $key => $value) {
			...
			$videoPubDate = $xml_doc->createElement('updateTimeUnix');
			$videoPubDate = $item->appendChild($videoPubDate);
			$videoPubDateCont = $xml_doc->createTextNode( round($value['c_ts_updated_l'] *1000 ) );
			$videoPubDateCont = $videoPubDate->appendChild($videoPubDateCont);
			...
			// $image = $contents->appendChild($image);
			// $imageU = $xml_doc->createElement('url');
			// $imageU = $image->appendChild($imageU);
			// $imageUCont = $xml_doc->createTextNode($thumb);
			// $imageUCont = $imageU->appendChild($imageUCont);
