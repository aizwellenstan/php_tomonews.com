<?php 
	...
	$getUrls2Now= APPLICATION_FEED_URL.'ARARPVJIEFTAEUCXARDA?start=0&count=100'.$dateChar.'&sort=c_ts_publish_l+desc';
	$ww=curl_info($getUrls2Now, '');
	$dAry22=json_decode($ww, true);
    $dAryn2=$dAry22['docs'];

    generate_xml($dAryn2, "id", $fileName);
...
function generate_xml($dAryn2, $lang, $typeContent){
		...
		$titleCont = $xml_doc->createTextNode($uuid.$dAryn2[0]['obj_id']);
