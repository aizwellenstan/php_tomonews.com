...
function VIDEO() {
	...
	anvp.listener = playerCallback;
		function playerCallback(event)	{
			...
			switch(event.name) {
				case "VIDEO_STARTED":
					_GLOBAL.old_ga != _GLOBAL.vdo_title + "-" + _GLOBAL.vdo_id, _GLOBAL.old_ga = _GLOBAL.vdo_title + "-" + _GLOBAL.vdo_id
					break;
...
function TBNS_PL() {
		...
		$.ajax({
		   ...
		   dataType: "json",
            success: function(t) {
				...
				return -1 == N && "0" != v ? void e(v) : ("0" == v && (N = 0), a > 0 ? (b = B.length, A = Math.ceil(b / 4)) : $(y).find(".thumbnail_item").css("opacity", 1), _(), 2 == M ? (T || (L = new VIDEO, L.set_playListen(), d(B[N], k[N], G[N], N)), s(), i()) : (M = 2, n(f, M, S)), void O())
    ...
    function _() {
        ...
        for (var t = (M - 1) * S; t < B.length; t++) $(y).append('<div class="thumbnail_item btn" data-id="' + G[t] + '"  data-title="' + B[t] + '"  data-desc="' + k[t] + '"   data-index="' + t + '" ><div class="thumbnail_img"style="background:url(\'' + w[t] + '\') ; background-size:cover;"></div><div class="thumbnail_title" style="bottom:-77px">' + unescape(B[t]) + '</div><div class="playingtxt invi"><img src="' + _GLOBAL.base + 'img/playing.png">Playing Now</div></div>');
...
function TBNS_PL_BOTTOM() {
		...
		$.ajax({
            ...
            success: function(t) {
				...
				for (var c = 0; c < s; c++) {
					...
					h.push(escape($tit));
					f.push($vid);
					v.push(escape(obj.docs[c].c_description_s )); 
    ...
    function a() {
        ...
        for (var t = (L - 1) * y; t < h.length; t++) {
            ...
            $(u).append('<div class="thumbnail_item btn" data-id="' + f[t] + '"  data-title="' + h[t] + '"  data-desc="' + v[t] + '"    ><div class="thumbnail_img"style="background:url(\'' + p[t] + "') ; background-repeat:no-repeat;background-size:contain;\"></div>" + i + '<div class="thumbnail_title" style=""><p>' + unescape(h[t]) + "</p></div></div>")
        }
    ...
    function o(t) {
        return $menuAry = [], $menuAry["4476"] = "U.S.", $menuAry["4477"] = "World", $menuAry["4478"] = "Asia", $menuAry["4485"] = "Sci & Tech", $menuAry["4479"] = "Sports", $menuAry["4480"] = "Entertainment", $menuAry["4481"] = "Politics", $menuAry["4482"] = "Geek", $menuAry["4483"] = "TomoGirls", $menuAry["4484"] = "Society", $menuAry["4486"] = "Chinese", $menuAry["No Comment"] = "4487", $menuAry["4489"] = "Tomo Originals", $menuAry[t]
    }
