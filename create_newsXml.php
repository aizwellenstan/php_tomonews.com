...
/// start 
	...
	$save_path = 'newsSitemaps/';
	
	$edate=strtotime(date('Y-m-d'));
	$sdate=strtotime(date('Y-m-d', strtotime('-2 days')));
	...
	if(file_exists($save_path.$fileName)){
		...
		foreach ($dAryn2['docs'] as $key => $value) {
			...
			$keywords = $value['c_tag_smv'];
			$urlN = THIS_SITE.urlencode(str_replace(' ', '-', remove_punc($title))).'-'.$value['obj_id']; 
			
			$f = $xml_doc->createDocumentFragment();
			$f -> appendXML("\n<url>\n  <loc>$urlN</loc>\n" .
					 "  <news>\n" .
					 "   <publication>\n" .
					 "    <name>TomoNews US</name>\n" .
					 "    <language>en</language>\n" .
			         "   </publication>\n" .
					 "   <publication_date>".date('c',time())."</publication_date>\n" .
					 "   <title><![CDATA[$title]]></title>\n" .
					 "   <keywords><![CDATA[".implode(",", $keywords)."]]></keywords>\n" .
			         "  </news>\n" .
					 "</url>\n");
		...
		$urlsetA = $xml_doc->createAttribute('xmlns:news');
		$urlsetA -> value = 'http://www.google.com/schemas/sitemap-news/0.9';
		$urlset->appendChild($urlsetA);
		$urlset = $xml_doc->appendChild($urlset);
